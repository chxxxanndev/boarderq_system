...
import {
  Menu, Bell, Settings, ChevronDown, LogOut,
  Home, LayoutGrid, Info, FileText, CreditCard,
  Wrench, Megaphone, AlertTriangle, Clock
} from 'lucide-react';
...
const NOTIF_CONFIG = {
  application: {
    ...
    getMessage: (n) => `${n.actor} applied for ${n.room_name}`,
  },
  payment: {
    icon: CreditCard,
    ...
    getMessage: (n) =>
      n.actor
        ? `${n.actor} submitted ₱${Number(n.amount).toLocaleString()} via ${n.method?.toUpperCase()} — ${n.room_name}`
        : `Your ₱${Number(n.amount).toLocaleString()} payment for ${n.room_name} was ${n.status}`,
  },
  flagged: {
    icon: AlertTriangle,
    color: 'bg-red-500',
    label: 'Flagged Payment',
    getMessage: (n) => `${n.actor}'s payment of ₱${Number(n.amount).toLocaleString()} was flagged — ${n.room_name}`,
  },
  maintenance: {
    icon: Wrench,
    color: 'bg-amber-500',
    ...
    getMessage: (n) =>
      n.actor
        ? `${n.actor} reported "${n.issue}" in ${n.room_name}`
        : `Your request "${n.issue}" in ${n.room_name} is now ${n.status?.replace('_', ' ')}`,
  },
  announcement: {
    icon: Megaphone,
    color: 'bg-violet-500',
    label: 'Announcement',
    getMessage: (n) => n.title,
  },
};
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-0.5">
            ...
            {n.status && (
              <span className={`text-[8px] font-black uppercase tracking-wide px-1.5 py-0.5 rounded-full ${STATUS_BADGE[n.status] ?? 'bg-gray-100 text-gray-500'}`}>
                {n.status.replace('_', ' ')}
              </span>
            )}
          </div>
          ...
          <span className="text-[9px] text-[#9CA3AF] flex items-center gap-1 mt-1">
            <Clock size={9} /> {timeAgo(n.date)}
          </span>
        </div>
      </div>
    </div>
        ...
        {!isDashboardPage && (
          <div className="flex items-center gap-6 md:gap-10">
            <Link href="/" className={`flex items-center transition-colors ${pathname === '/' ? 'text-[#1E5EFF]' : 'text-[#6B7280] hover:text-[#0B1F3B]'}`}>
              ...
              <span className="hidden md:block text-[11px] font-black uppercase tracking-[0.2em]">Overview</span>
            </Link>
            <Link href="/public/rooms" className={`flex items-center transition-colors ${pathname === '/public/rooms' ? 'text-[#1E5EFF]' : 'text-[#6B7280] hover:text-[#0B1F3B]'}`}>
              ...
              <span className="hidden md:block text-[11px] font-black uppercase tracking-[0.2em]">Browse Rooms</span>
            </Link>
            <Link href="/public/about" className={`flex items-center transition-colors ${pathname === '/public/about' ? 'text-[#1E5EFF]' : 'text-[#6B7280] hover:text-[#0B1F3B]'}`}>
              ...
              <span className="hidden md:block text-[11px] font-black uppercase tracking-[0.2em]">About</span>
            </Link>
          </div>
        )}
                    ...
                    <div className="max-h-[420px] overflow-y-auto">
                      {notifications.length > 0 ? (
                        notifications.map((n, i) => (
                          <NotifItem key={`${n.type}-${n.id}-${i}`} n={n} />
                        ))
                      ) : (
                        <div className="px-4 py-10 text-center">
                          <Bell size={28} className="text-[#E5E7EB] mx-auto mb-2" />
                          <p className="text-[10px] font-black uppercase tracking-widest text-[#9CA3AF]">All caught up</p>
                        </div>
                      )}
                    </div>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-3 w-52 md:w-60 bg-white border border-[#E5E7EB] rounded-xl shadow-2xl py-2 z-[110]">
                  ...
                  <div className="h-px bg-[#E5E7EB] my-1 mx-3" />
