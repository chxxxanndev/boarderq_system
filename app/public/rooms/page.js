                  ...
                  <div className="mt-auto pt-6 border-t border-[#F8FAFC] flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                         ...
                         <button
                            ...
                            className="w-14 h-14 bg-[#0B1F3B] text-white rounded-2xl flex items-center justify-center hover:bg-[#1E5EFF] transition-all shadow-lg active:scale-95"
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
            <div className="space-y-4">
              <Link href="/"><h3 className="text-2xl font-black tracking-tighter uppercase text-[#0B1F3B]">BOARDER<span className="text-[#1E5EFF] italic">Q</span></h3></Link>
              <p className="text-[11px] text-[#6B7280] font-bold uppercase tracking-wider leading-relaxed">Redefining housing experience through automated logistics.</p>
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 bg-[#F8FAFC] border border-[#E5E7EB] rounded-lg flex items-center justify-center text-[#6B7280]"><Globe size={18} /></div>
                <div className="w-9 h-9 bg-[#F8FAFC] border border-[#E5E7EB] rounded-lg flex items-center justify-center text-[#6B7280]"><Mail size={18} /></div>
              </div>
            </div>
            <div>
              <h4 className="text-[10px] font-black text-[#0B1F3B] uppercase tracking-[0.4em] mb-6">Ecosystem</h4>
              <ul className="space-y-3">
                {[{label:'Overview', href:'/'}, {label:'Browse Rooms', href:'/public/rooms'}, {label:'About', href:'/public/about'}, {label:'Sign In', href:'/public/login'}].map(link => (
                  <li key={link.label}><Link href={link.href} className="text-[11px] font-bold text-[#6B7280] hover:text-[#1E5EFF] uppercase tracking-widest transition-colors flex items-center gap-2 group"><div className="w-1 h-1 bg-[#E5E7EB] group-hover:bg-[#1E5EFF]" /> {link.label}</Link></li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <div className="space-y-4">
                <a href="https://www.facebook.com/che.ann.abal.2024" target="_blank" rel="noopener noreferrer" className="flex items-start gap-3 group">
                  <Globe className="text-[#1E5EFF] mt-1" size={18} /><p className="text-[11px] font-black text-[#0B1F3B] uppercase tracking-tight group-hover:text-[#1E5EFF]">Official Facebook</p>
                </a>
              </div>
            </div>
            <div>
              ...
              <div className="bg-[#F8FAFC] border border-[#E5E7EB] rounded-2xl p-4 flex items-start gap-3">
                <MapPin size={18} className="text-[#1E5EFF]" /><p className="text-[11px] font-bold text-[#6B7280] uppercase tracking-wide leading-relaxed">Dapitan City, ZN, 7101<br />Philippines</p>
              </div>
            </div>
          </div>
          <div className="pt-6 border-t border-[#F1F5F9] flex justify-between items-center text-[9px] font-black text-[#CBD5E1] uppercase tracking-[0.4em]">
            <span>BOARDER-Q © 2026</span>
            <Link href="/public/login" className="text-[#1E5EFF] hover:underline">System Access</Link>
          </div>
        </div>
      </footer>
    </div>
